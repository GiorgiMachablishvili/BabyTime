...
final class VaccinationReminderEditViewController: UIViewController {

    var reminder: VaccinationReminder?
    var forDate: Date?
    var onSave: ((VaccinationReminder) -> Void)?
    ...
    private func updateButtonAvailability() {
        let visitDate = reminder?.date ?? forDate ?? Date()
    ...
    private func buildDayButtons() {
        for days in VaccinationReminder.notifyDaysOptions.reversed() {
            let b = UIButton(type: .system)
    ...
    @objc private func saveTapped() {
        ...
        let updated = VaccinationReminder(
            id: id,
            date: day,
            hour: hour,
            minute: minute,
