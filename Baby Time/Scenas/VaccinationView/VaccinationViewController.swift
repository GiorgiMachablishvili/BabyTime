...
final class VaccinationViewController: UIViewController {
    ...
    private lazy var addVaccinationView: AddVaccinationView = {
        let view = AddVaccinationView()
        view.isHidden = true
        view.onTapCloseButton = { [weak self] in
            guard let self else { return }
            UIView.animate(withDuration: 0.3, delay: 0, options: [.curveEaseInOut]) {
                self.addVaccinationView.transform = CGAffineTransform(translationX: 0, y: self.view.bounds.height)
            } completion: { _ in
                self.addVaccinationView.isHidden = true
            }
        }
        return view
    }()
    ...
    private func setupUI() {
        ...
        view.addSubview(addVaccinationView)
    }
    ...
    private func setupConstraints() {
        ...
        addVaccinationView.snp.makeConstraints { make in
    ...
    private func configureViews() {
        ...
        emptyStateView.configure(
            icon: UIImage(systemName: "syringe"),
            ...
            subtitle: "Tap the + to add a vaccination or calendar for reminders"
        )
    }

    private func presentAddVaccination() {
        addVaccinationView.isHidden = false
        addVaccinationView.transform = CGAffineTransform(translationX: 0, y: view.bounds.height)
        addVaccinationView.onTapSave = { [weak self] visitDate, hour, minute, note in
            guard let self else { return }
        ...
        UIView.animate(withDuration: 0.35, delay: 0, usingSpringWithDamping: 0.9, initialSpringVelocity: 0.6, options: [.curveEaseInOut]) {
            self.addVaccinationView.transform = .identity
        }
    }
...
extension VaccinationViewController: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
